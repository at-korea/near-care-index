...
**Current version: v0.7** (2026-05-04) — 가중치판
...
├── LICENSE                  ← MIT
├── data/
│   ├── near_v0.7.json       ← 메인 데이터 (229 시군구 × N/R/A/E + IC + 순위)
│   ├── near_v0.7.csv        ← CSV 형태 (대체)
│   ├── metadata.json        ← 버전·출처·changelog
│   ├── pillars.json         ← 4축 정의 + 가중치 + 색상 팔레트
...
## Pillar Structure (v0.7 가중치)

| Pillar | Korean | Question | Weight | Color |
|---|---|---|---|---|
| **N** Needs | 욕구·필요 | 누가 얼마나 돌봄이 필요한가 | **−0.6** (페널티) | `#5E60CE` |
| **R** Resources | 자원·공급역량 | 어떤 돌봄 자원이 얼마나 있는가 | **0.45** | `#06A877` |
| **A** Access | 접근·접근성 | 자원에 닿을 수 있는가 | **0.30** | `#E8801E` |
| **E** Engagement | 결속·지역참여 | 함께 작동하는가 | **0.25** | `#C44569` |

**R sub-pillar 가중치 (v0.7)**
- R_KCCI 0.5 + R3_integratedCare 0.5

**Composite formula (v0.7 가중치판)**
```
NEAR_A = clip( 0.45·R + 0.30·A + 0.25·E − 0.6·N + 50, 0, 100 )
```

**민감도**: ±15% 가중치 변동 시 분류 안정성 96.4% (DSAR v0.5의 89.2% 대비 +7.2%p)
...
```javascript
// Main data
const res = await fetch('./data/near_v0.7.json');
...
//   regionName: '시흥시',
//   sido: '경기',
//   scores: { N: 26.05, R: 34.79, A: 86.20, E: 55.61, NEAR_A: 96.4 },
//   rank: 1,
//   quadrant: 'II',
//   ...
// }
...
console.log(q.quadrants.IV.count);   // 89 (돌봄위기형)
console.log(q.thresholds.N_median);  // 45.27
```
...
```json
{
  ...
  "count": 229,
  "thresholds": { "N_median": 45.27, "Resource_median": 35.56 },
  "formula": "0.45*R + 0.30*A + 0.25*E - 0.6*N + 50",
  "rPillarSubWeights": { "R_KCCI": 0.5, "R3_integratedCare": 0.5 },
  "regions": [
    {
      "regionCode": "11090",
      "regionName": "강북구",
      ...
      "issuer": true,
      "scores": {
        "N": 39.48, "R": 22.68, "A": 69.80, "E": 46.13,
        "resource": 38.48, "NEAR_A": 64.79
      },
      "subPillars": {
...
## Quadrant Distribution (v0.7 기준)

| Quadrant | Label | Count | Policy |
|---|---|---|---|
| **I** | 균형돌봄선진형 | 26 | 모범사례·거점 |
| **II** | 안정자원풍부형 | 89 | 미래 대비 |
| **III** | 일반저욕구형 | 25 | 모니터링 |
| **IV ★** | **돌봄위기형** | **89** | **즉시 개입 1순위** |
...
- **MOHW** — 보건복지부 (통합돌봄 사업 stats.json, v0.7 가중 강화)
...
| **v0.7** | **가중치판 (R sub 0.5/0.5, Composite 0.45/0.30/0.25, N −0.6)** | **2026-05-04** |
...
```
KCCI Phase 2-C Team (2026). NEAR Care Index v0.7:
Korea Community Care Capacity Index for 229 Sigungu (Weighted).
https://github.com/at-korea/near-care-index
```
